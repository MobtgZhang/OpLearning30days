/* 改编自 30dayMakeOS sheet.c：UINT32 缓冲区 + pitch 行间距 */
#include "sheet.h"
#include <stddef.h>
...
{
	int h, bx, by, vx, vy, bx0, by0, bx1, by1;
	UINT32 *buf, *vram = ctl->vram, sid;
	UINT8 *map = ctl->map;
	UINTN pitch = ctl->pitch_px;
	struct SHEET *sht;
